...
validate.validatePath(pathToTargetDir)
...
def mycompareRunOrder(x,y,pathToTargetDir):
  fn = "errors"
  errX = os.path.join(os.path.join(pathToTargetDir, x), fn)
  errY = os.path.join(os.path.join(pathToTargetDir, y), fn)
  if os.path.isfile(errX) and os.path.isfile(errY):
    return cmp(os.stat(errX)[8],
               os.stat(errY)[8])
  else:
    try:
      return cmp(os.stat(os.path.join(pathToTargetDir, x))[8], os.stat(os.path.join(pathToTargetDir, y))[8])
    except:
      return 0

def mycompareCompDuration(x,y,pathToTargetDir):
  fn = "gmake_call"
  befX = os.path.join(os.path.join(pathToTargetDir, x), fn)
  befY = os.path.join(os.path.join(pathToTargetDir, y), fn)
  fn = "gmake_output"
  aftX = os.path.join(os.path.join(pathToTargetDir, x), fn)
  aftY = os.path.join(os.path.join(pathToTargetDir, y), fn)
  if os.path.isfile(aftX) and os.path.isfile(aftY) and  os.path.isfile(befX) and os.path.isfile(befY):
    return cmp(os.stat(aftX)[8]-os.stat(befX)[8],
               os.stat(aftY)[8]-os.stat(befY)[8])
  else:
    try:
      return cmp(os.stat(os.path.join(pathToTargetDir, x))[8], os.stat(os.path.join(pathToTargetDir, y))[8])
    except:
...
def myLastDirStat8(pathToTargetDir, x):
  testdir=os.path.join(pathToTargetDir, x)
  baselineFn = os.path.join(testdir, "test.info")
  if os.path.isfile(baselineFn):
    ans = os.stat(baselineFn)[8]
  else:
    ans = os.stat(testdir)[8]
  items = os.listdir(testdir)
  for item in items:
    fullitemdir = os.path.join(testdir, item)
    if os.path.isdir(fullitemdir):
      fullitem = os.path.join(fullitemdir, "run_summary")
      lastdirtime = os.stat(fullitem)[8]
      if lastdirtime > ans:
        ans = lastdirtime
      fullitem = os.path.join(fullitemdir, "flash_output")
      lastdirtime = os.stat(fullitem)[8]
      if lastdirtime > ans:
        ans = lastdirtime
  return ans

def mycompareDuration(x,y,pathToTargetDir):
  fn = "test.info"
  befX = os.path.join(os.path.join(pathToTargetDir, x), fn)
  befY = os.path.join(os.path.join(pathToTargetDir, y), fn)
  fn = "errors"
  ...
  aftY = os.path.join(os.path.join(pathToTargetDir, y), fn)
  if os.path.isfile(aftX) and os.path.isfile(aftY) and  os.path.isfile(befX) and os.path.isfile(befY):
    return cmp(myLastDirStat8(pathToTargetDir,x)-os.stat(befX)[8],
               myLastDirStat8(pathToTargetDir,y)-os.stat(befY)[8])
  else:
    try:
      return cmp(os.stat(os.path.join(pathToTargetDir, x))[8], os.stat(os.path.join(pathToTargetDir, y))[8])
    except:
...
def mycompareRunDuration(x,y,pathToTargetDir):
  fn = "compilation_time"
  befX = os.path.join(os.path.join(pathToTargetDir, x), fn)
  befY = os.path.join(os.path.join(pathToTargetDir, y), fn)
  fn = "errors"
...
if orderBy == "timestamp":
  ...
  #  items.sort(lambda x,y: cmp(os.stat(os.path.join(pathToTargetDir, y))[9], os.stat(os.path.join(pathToTargetDir, x))[9]))
  items.sort(lambda x,y: mycompareRunOrder(x,y,pathToTargetDir))
elif orderBy == "duration":
  items.sort(lambda x,y: mycompareDuration(x,y,pathToTargetDir))
elif orderBy == "runduration":
  items.sort(lambda x,y: mycompareRunDuration(x,y,pathToTargetDir))
elif orderBy == "compduration":
  items.sort(lambda x,y: mycompareCompDuration(x,y,pathToTargetDir))
else:
  items.sort()
...
allTestsList      = list(allTests.keys())
infoFileTestsList = list(infoFileTests.keys())
allTestsList.sort()
