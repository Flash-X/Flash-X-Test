...
class LayeredDict:
  ...
  many of the methods of a python3 dictionary.
...
#    print("In contains with key, self",key,self)
    if key in self:
      return True
    return False
  ...
  def __getitem__(self, key):
    ...
    n = self.activeLayer
    while n >= 0:
      if key in self.dicts[n]:
        val = self.dicts[n][key]
        return self.dereferencePointers(val)
      else:
        n -= 1
    raise KeyError
...
#    print("This is the string", s)
  ...
  def dereferencePointers(self, val):
