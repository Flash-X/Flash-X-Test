function CompositeSubmit() {
  ...
  for (i=0;i<document.updateType.testType.length;i++){
    if(document.updateType.testType[i].checked){
      testType = document.updateType.testType[i].value;
      break;
    }
  }  

   // if (testerClasses[0] == "Composite") {
     if(testType == "comparison"){
      newKey = "comparisonBenchmark";
      newValue = "<siteDir>/" + selectedDate + "/<buildDir>/<runDir>/<checkpointBasename><comparisonNumber>";
     }
     else if(testType == "restart"){
       newKey = "restartBenchmark";
       newValue = "<siteDir>/" + selectedDate + "/<buildDir>/<runDir>/<checkpointBasename><restartNumber>";
     }		
     // }
